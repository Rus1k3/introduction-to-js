let flying_nlo = {
    ...
    beam: function(){
        ...
        beginShape();
        vertex(this.x - this.width/12, this.y + 10);
        vertex(this.x + this.width/12, this.y + 10);
        vertex(this.x + this.width/5, height - 100);
        vertex(this.x - this.width/5, height - 100);
        endShape();
    },

    // Проверка на попадание коровы в луч
    catch_cow: function(cow) {
        return cow.x > this.x - this.width / 4 && cow.x < this.x + this.width / 4;
    }
}
...
function CowManager()
{
    ...
    this.update = function()
    {
        ...
        for (let i = 0; i < cows.length; i++)
        {
            ...
            //проверка на поимку коровы
            if (flying_nlo.catch_cow(cows[i])) {
                cows.splice(i, 1);
                cowCounter++;
        }
    }
...
function draw(){
    ...
    // отображение счетчика коров
    fill(255);
    textSize(24);
    text("Коров Собрано: " + cowCounter, 10, 30);
